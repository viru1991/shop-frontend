...
const ColorPicker = forwardRef(
  (
    ...
    const handleSelect = useCallback(
      (color, isAvailable) => {
        if (!isAvailable) return; // ✅ ignore clicks on unavailable colors
    ...
    return (
      ...
      >
        {colors.map((color) => {
          const isAvailable = availableColors.includes(color); // ✅ NEW
          const hasSelected = singleSelect ? selected === color : selected.includes(color);
          ...
          return (
            <ButtonBase
              ...
              sx={{
                ...
                height: 36,
                borderRadius: '50%',
                cursor: isAvailable ? 'pointer' : 'not-allowed', // ✅ NEW
                opacity: isAvailable ? 1 : 0.4, // ✅ NEW
              }}
              onClick={() => {
                handleSelect(color, isAvailable); // ✅ NEW
              }}
            >
              <Stack
                alignItems="center"
...
ColorPicker.propTypes = {
  colors: PropTypes.oneOfType([PropTypes.string, PropTypes.arrayOf(PropTypes.string)]),
  availableColors: PropTypes.arrayOf(PropTypes.string), // ✅ NEW
  limit: PropTypes.number,
  onSelectColor: PropTypes.func,
  selected: PropTypes.oneOfType([PropTypes.string, PropTypes.arrayOf(PropTypes.string)]),
  sx: PropTypes.object,
